...
2. **Learning Phases Workflow Diagram**:
   - Provide a visual flowchart using **Mermaid syntax** (fenced in a ```mermaid code block) representing the progression of learning phases and key milestones.
...
4. **Official Reference Roadmaps from roadmap.sh**:
...
5. **General Upskilling Advice**: Tips on portfolio building, resume integration, and job search strategies.

Be encouraging, specific, and practical. Ensure all Mermaid code blocks are syntactically valid.
"""
...
2. **DSA Learning Pathway Diagram**:
   - Provide a visual learning path flowchart using **Mermaid syntax** (fenced in a ```mermaid code block) showing the progression of topics (e.g., Arrays -> Linked Lists -> Trees -> Graphs -> Dynamic Programming).
3. **Weekly/Phased Schedule Table**:
   - Provide a structured Markdown table mapping out:
     - Phase / Week
     - Core Topics (e.g., Arrays, Two Pointers, DFS, DP)
     - Key Problem Patterns (e.g., Sliding Window, Tree Traversals)
     - Difficulty Target (e.g., LeetCode Easy/Medium)
...
1. **Headline Recommendations**:
   - Provide a Markdown table showing 3 optional high-impact, SEO-optimized headlines. The table should compare:
     - Headline Option
     - Key Focus / Value Proposition
     - Recruiters' Target Keywords
...
3. **Work Experience Refinements**:
   - Provide a Markdown table showing:
     - Original Highlight (from Resume)
     - Optimized LinkedIn Bullet Point (focused on impact, metrics, and active voice)
...
2. **Architecture & Data Flow Diagram**:
   - Provide a system design flowchart using **Mermaid syntax** (fenced in a ```mermaid code block) visualizing how components communicate (e.g., Client -> API Gateway -> Backend microservices -> DB / Cache).
3. **Target Technical Stack**: Recommendations for frontend, backend, databases, libraries, and dev tools.
4. **Key Features & Requirements**: 4-5 bullet points of core functionalities.
